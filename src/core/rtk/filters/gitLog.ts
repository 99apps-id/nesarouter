...
export function gitLog(text, maxLines = GIT_LOG_MAX_LINES) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // commit <sha> header â€” starts new commit entry
    // Also matched with leading graph decoration (`*   commit abc1234...` â€” --graph without --oneline)
    ...
    if (inCommit) {
      // Author / Date â€” keep as-is (already column 0 in raw, or graph-prefix stripped by commit-header match)
      ...
      // blank â€” skip
      if (trimmed === "") continue;
      // indented subject (4 spaces, optionally preceded by graph decoration) â€” first one is subject
      ...
      // embedded diff header â€” one-line marker
      if (/^diff --git /.test(trimmed)) {
        pushLine("  ... diff body omitted");
        continue;
      }
      // everything else in commit body â€” drop
      continue;
    }
    ...
    // Pure graph decoration (no sha) â€” drop
